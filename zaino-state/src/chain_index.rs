...
impl<Source: BlockchainSource> NodeBackedChainIndex<Source> {
    ...
    async fn finalize_blocks_up_to_tip(
        ...
        loop {
            // Get current DB height (None if empty)
            let current_db_height = fs
                .to_reader()
                .db_height()
                .await
                .map_err(|_e| SyncError::CannotReadFinalizedState)?;

            // Calculate next height to finalize and effective DB height for comparison.
            // When DB is empty (None), we start at Height(0) and use 0 for comparison.
            // When DB has blocks, we start at db_height + 1.
            let (next_finalized_height, db_height_for_comparison) = match current_db_height {
                Some(height) => (height + 1, height.0),
                None => (types::Height(0), 0),
            };

            // Keep non-finalized state at most 100 blocks ahead of finalized
            if nfs_tip_height <= db_height_for_comparison + 100 {
                break;
            }
