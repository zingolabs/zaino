...
**Org ledger**:
The `zingolabs/zingo-adrs` repository: the one append-only ledger of
architecture decision records for every zingolabs code repository. Code
repositories carry a mirrored copy of the whole ledger, never a subset.
_Avoid_: zaino-adrs, "the ADR repo" for any other location

**Org-scoped record**:
A record that binds every zingolabs code repository. It lives at the top
level of the org ledger.
_Avoid_: global ADR, shared ADR

**Repo-scoped record**:
A record that binds exactly one code repository. It lives in that
repository's subdirectory of the org ledger (`zaino/` for zaino).
_Avoid_: local ADR, project ADR

**Record citation**:
Each scope of the org ledger numbers its records in its own sequence.
