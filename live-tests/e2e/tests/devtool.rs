...
///
/// The arithmetic over Zaino's own entries is covered by mocks in
/// `zaino-mempool-rpc` (`get_mempool_info_reports_totals`); recomputing it here
/// from those same entries would only assert that Zaino equals itself. What a
/// live validator adds is the cross-check: Zaino's mirror of the mempool must
/// report the same transaction count and byte total the validator does.
async fn get_mempool_info_fetch() {
    ...
    let validator_info = test_manager
    ...
    // (each transaction floored at the cost threshold), which is the figure its
    // own memory bound is enforced against, whereas the validator reports its
    // internal heap estimate. They are different quantities by design.
...
///
/// Worth running on both backends because the mempool is served over JSON-RPC in
/// *either* case (the Direct backend still reads it through `mempool_fetcher`),
/// so this pins that the backend choice does not change the reported totals.
async fn get_mempool_info_state() {
    let mut svc = fund_and_fill_mempool_dual().await;

    let info = svc.state_subscriber.get_mempool_info().await.unwrap();
    let validator_info = svc
        .test_manager
        .full_node_jsonrpc_connector()
        .await
        .get_mempool_info()
    ...
    // See `get_mempool_info_fetch`: `usage` is Zaino's ZIP-401 cost total, not
    // the validator's heap estimate.
    assert!(info.usage >= info.bytes);
