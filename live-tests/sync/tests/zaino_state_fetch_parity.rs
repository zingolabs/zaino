...
use ztest::sync::{
    Severity, Snapshot, SyncCtx, SyncOutcome, SyncRunner, Verdict, Violation, hours, mins, secs,
};
...
///
/// - Default 20 s = a regtest number; the direct pod opens a 257 GiB state dir before its
///   gRPC binds
...
async fn zaino_state_fetch_parity(mut run: SyncRunner) -> SyncOutcome {
        ...
        .check(subject_index_append_only);
    {
        ...
        run.always(Severity::Fatal).named("frontiers_within_pin").every(secs(30)).check_rpc(
            move |s, _cx| {
                let zaino_state = zaino_state.clone();
    ...
    {
        let zaino_state = zaino_state.clone();
        run.sometimes().named("observed_both_mid_build").check_rpc(move |s, _cx| {
            let zaino_state = zaino_state.clone();
    ...
    {
        let zaino_state = zaino_state.clone();
        run.sometimes().named("both_answered_from_their_own_index").check_rpc(move |s, cx| {
            let zaino_state = zaino_state.clone();
            Box::pin(async move { both_answered_from_their_own_index(s, cx, &zaino_state).await })
        });
    }
    ...
    {
        let zaino_state = zaino_state.clone();
        run.at_completion(Severity::Fatal).named("indexes_converged").check_rpc(move |s, _cx| {
            let zaino_state = zaino_state.clone();
    ...
    {
        let zaino_state = zaino_state.clone();
        run.at_completion(Severity::Fatal).named("indexes_agree_at_the_pinned_tip").check_rpc(
            move |s, cx| {
                let zaino_state = zaino_state.clone();
                Box::pin(async move {
...
async fn both_answered_from_their_own_index(
    ...
    match direct_rpc.call_value("getaddressdeltas", selector.clone()).await {
        Ok(_) => {}
...
async fn compare_indexes(
    ...
    for height in heights {
        ...
        for params in [json!([height.to_string(), 0]), json!([height.to_string(), 1])] {
...
mod tests {
    ...
    fn both_ends_of_the_window_are_always_sampled() {
        for window in [1, 2, 9, 1_000, 3_434_143, 4_140_000] {
            ...
            assert!(heights.contains(&window), "window {window} skipped the frontier");
        }
    }
