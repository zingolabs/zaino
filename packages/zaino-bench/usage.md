...
Completion is derived from those two heights, deliberately. The node's own
`zaino.sync.has_reached_tip` and `zaino.sync.lag_blocks` gauges were the obvious
alternative and neither meant what its name suggested:

- `has_reached_tip` was set when the sync loop's iteration returned `Ok`, and
  ...
  no-op). It went to 1 seconds after start-up and stayed there — it meant "the
  sync loop is healthy", not "the index is at the tip".
- `lag_blocks` was `chain_tip - finalized_height_floor(chain_tip)`: the
  non-finalised seam depth, a constant, not the distance left to sync.

Both have since been removed from the node for those reasons, so the derivation
above is now the only reading. Liveness is `zaino.sync.iterations_total{outcome}`
with `zaino.sync.consecutive_failures`; sync lag is
`zaino.chain.tip_height - zaino.sync.finalized_height`, left to the consumer.
The summary's `reached_tip` and the CSV's `node_lag_gauge` column are therefore
empty for a run against a current node.
