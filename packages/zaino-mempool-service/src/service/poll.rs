//! The background poll task: diff the source against the held set and drive the
//! reconcile / republish decisions.
...
impl<S: MempoolSource> super::MempoolService<S> {
    ...
    fn metadata_fetch_is_due(&self, state: &PollState) -> bool {
        match state.last_metadata_fetch {
            Some(last) => last.elapsed() >= self.config.metadata_min_interval.get(),
            None => true,
        }
    }
    ...
    async fn fetch_added_entries_bounded(
        ...
        let concurrency = self.config.max_concurrent_raw_fetches.get();
