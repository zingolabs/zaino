...
// =========================================================================
// Destination traits — split the ChainIndex API into focused interfaces.
// ChainIndex itself is the supertrait below.
// =========================================================================

/// Base trait: associated types shared by all destination traits.
/// Separated to break the supertrait cycle — `Validator`, `CompactBlockPublisher`,
/// and `Mempool` all depend on `ChainIndexBase` for `Snapshot` and `Error`.
pub trait ChainIndexBase {
...
/// Validator: block, transaction, chain, address, and metadata queries.
///
/// Methods that query blockchain state — block lookups, transaction status,
/// chain navigation, transparent address history, and UTXO-set statistics.
/// Some methods take a `&Self::Snapshot` for height-indexed queries that
/// need a consistent view of the non-finalized chain tip.
pub trait Validator: ChainIndexBase {
...
/// CompactBlockPublisher: compact block serving for light wallets.
///
/// Methods that produce [`CompactBlock`] payloads for the `GetBlock` and
/// `GetBlockRange` gRPC endpoints. These are the read path for light-wallet
/// clients performing block scanning.
pub trait CompactBlockPublisher: ChainIndexBase {

    /// Returns the *compact* block for the given height.
    ///
    /// Returns `None` if the specified `height` is greater than the chain tip.
    ///
    /// ## Pool filtering
    ///
    ...
    ///
    /// Returns `None` if the requested range is entirely above the chain tip.
    ///
...
/// Mempool: in-flight transaction queries.
///
/// Methods that access the mempool — transactions not yet included in a
/// block. These are independent of chain state (except for the optional
/// chain-tip check on `get_mempool_stream`).
pub trait MempoolAccess: ChainIndexBase {
...
///
/// `ChainIndex` is the supertrait of [`Validator`], [`CompactBlockPublisher`],
/// and [`Mempool`] — anything that implements `ChainIndex` also implements
/// all three destination traits.
///
/// The only method that lives on `ChainIndex` itself is
/// [`snapshot_nonfinalized_state`] — the entry point for taking a consistent
/// point-in-time view of the non-finalized chain tip.
///
/// When a call asks for info (e.g. a block), Zaino selects sources in this order:
#[doc = simple_mermaid::mermaid!("chain_index_passthrough.mmd")]
pub trait ChainIndex: ChainIndexBase + Validator + CompactBlockPublisher + MempoolAccess {
...
pub struct NodeBackedChainIndex<Source: BlockchainSource = ValidatorConnector> {
    ...
    /// Block store — shadow ingestion target. Updated by the block-store
    /// sync loop alongside the existing finalized+NFS ingestion path.
    /// Serves [`CompactBlockPublisher`] queries via [`ChainState`].
    pub(super) block_store: std::sync::Arc<zaino_store::ChainState>,
    #[allow(dead_code)]
    block_store_handle: Option<tokio::task::JoinHandle<()>>,
}

// Manual Debug impl: skip BlockStoreSync (not Debug).
impl<Source: BlockchainSource + std::fmt::Debug> std::fmt::Debug for NodeBackedChainIndex<Source> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NodeBackedChainIndex")
            .field("mempool", &self.mempool)
            .field("non_finalized_state", &self.non_finalized_state)
            .field("finalized_db", &self.finalized_db)
            .field("sync_loop_handle", &self.sync_loop_handle)
            .field("status", &self.status)
            .field("network", &self.network)
            .field("source", &self.source)
            .field("sync_timings", &self.sync_timings)
...
impl<Source: BlockchainSource> NodeBackedChainIndex<Source> {
    ...
    pub(crate) async fn new_with_sync_timings(
        ...
        tracing::info!("Creating finalized DB...");
        let finalized_db =
            Arc::new(finalised_state::ZainoDB::spawn(config.clone(), source.clone()).await?);
        tracing::info!("Finalized DB created");
        let mempool_state = mempool::Mempool::spawn(source.clone(), None)
            .map_err(crate::InitError::MempoolInitialzationError)
            .await?;
        tracing::info!("Mempool created");

        // Block store: persistent ingestion target.
        let zebra_network = config.network.to_zebra_network();
        let default_start_height = zebra_chain::parameters::NetworkUpgrade::Sapling
            .activation_height(&zebra_network)
            .expect("Sapling activation height must be set")
            .0;
        let start_height = config.start_height.unwrap_or(default_start_height);
        tracing::info!(start_height, "Opening block store...");
        let block_store = std::sync::Arc::new(
            zaino_store::ChainState::open(
                &config.storage.database.path.join("block_store"),
                start_height,
            )
            .expect("Block store LMDB open failed, aborting"),
        );
        tracing::info!("Block store opened, starting sync loop...");
        let block_store_fetcher = crate::backends::block_store::ChainSourceFetcher::new(
            source.clone(),
            zebra_network.clone(),
        )
        .with_max_concurrency(config.block_store_max_concurrency);
        let block_store_timings = zaino_store::sync::SyncTimings::default();
        let block_store_sync = zaino_store::sync::BlockStoreSync::new(
            block_store.clone(),
            block_store_fetcher,
            block_store_timings,
        );
        let block_store_handle = Some(block_store_sync.start_sync_loop());
        tracing::info!("Block store sync loop started");
        tracing::info!("Returning ChainIndex...");
        ...
        // BlockStoreSync (started above) handles ongoing ingestion in the
        // background. All components (finalized_db, mempool, block_store) are
        // synchronously initialised and ready, so the ChainIndex starts Ready.
    ...
    ///
    /// Computed on the fly from component statuses. The ChainIndex's own status
    /// (`Ready` after construction, `Closing` after shutdown) is combined with
    /// the finalized DB and mempool statuses — the most severe wins. Does NOT
    /// mutate stored state, so a transient `Spawning` from a component does not
    /// permanently poison the aggregate.
...
impl<Source: BlockchainSource> NodeBackedChainIndexSubscriber<Source> {
    ...
    ///
    /// Computed on the fly — does not mutate stored state.
    /// See [`NodeBackedChainIndex::status`].
...
impl<Source: BlockchainSource> ChainIndexBase for NodeBackedChainIndexSubscriber<Source> {
...
impl<Source: BlockchainSource> Validator for NodeBackedChainIndexSubscriber<Source> {
...
impl<Source: BlockchainSource> CompactBlockPublisher for NodeBackedChainIndexSubscriber<Source> {
async fn get_compact_block(
    &self,
    height: types::Height,
    pool_types: PoolTypeFilter,
) -> Result<Option<zaino_proto::proto::compact_formats::CompactBlock>, Self::Error> {
    self.block_store.get_compact_block(height, pool_types).await
}
async fn get_compact_block_stream(
    ...
    self.block_store.get_compact_block_stream(start_height, end_height, pool_types).await
}
}

impl<Source: BlockchainSource> MempoolAccess for NodeBackedChainIndexSubscriber<Source> {
