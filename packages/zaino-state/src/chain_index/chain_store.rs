...
// ---------------------------------------------------------------------------
//
// Every finalised read ChainIndex makes goes through one of the helpers below,
...
use zaino_chain_store::{
    ChainStoreError, ChainStoreIngest, ChainStoreReader, ChainStoreSourceError, CompactBlockRead,
    SpentOutputIndex, StoredBlockRead, StoredTxOut, TransactionIndex, TxOutSetAccumulator,
    TxOutSetIndex,
};
...
///
/// Shares [`super::chain_head::domain_height`] rather than restating it: both
/// halves must agree about which heights are expressible, and a second copy is
/// a second thing to keep in step.
fn domain_height(height: crate::Height) -> Option<zaino_primitives::types::Height> {
    super::chain_head::domain_height(height)
}
...
pub(crate) async fn block_height<R: ChainStoreReader>(
    ...
    Ok(absent(
        reader
            .block_height(super::chain_head::domain_hash(hash))
            .await,
    )?
    .map(local_height))
}
