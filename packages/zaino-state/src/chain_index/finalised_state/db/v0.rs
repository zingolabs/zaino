...
impl DbV0 {
    ...
    pub(crate) async fn write_block(&self, block: IndexedBlock) -> Result<(), FinalisedStateError> {
        ...
        let zebra_height: ZebraHeight = block.chain_block().index.height.into();
        let zebra_hash: ZebraHash = zebra_chain::block::Hash::from(block.chain_block().index.hash);
        ...
        let block_height = block.chain_block().index.height.0;
