...
async fn fill_sync_batch<S: crate::chain_index::source::BlockchainSource>(
        ...
        && started.elapsed() < build.interval
    {
        ...
        for block in assembled {
            ...
            batch.push(block);
            cursor.next += 1;
            // The in-memory build frontier, per block, so it moves between the per-batch
            // commits that step `finalized_height`. Writer-only: the NFS loop runs to the tip
            // and would make the frontier retreat next pass.
            #[cfg(feature = "prometheus")]
