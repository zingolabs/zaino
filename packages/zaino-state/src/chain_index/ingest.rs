...
//!
//! - One writer, labelled `finalised`; a migration backfill re-reads under `migration`
//!
...
//! - `assemble` runs only on a fetched block → counts track, and `assemble` trails by
//!   the fetches whose assembly errored (timed on every exit path, fetch only on success)
