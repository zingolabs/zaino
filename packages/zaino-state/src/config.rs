...
/// Configuration for the [`NodeBackedIndexerService`](crate::NodeBackedIndexerService), which reaches its validator over JSON-RPC.
#[derive(Debug, Clone)]
...
impl CommonBackendConfig {
    /// Builds a [`CommonBackendConfig`], applying the default RPC user/password placeholder (`"xxxxxx"`) and this crate's `CARGO_PKG_VERSION` as the indexer version.
