//! The JSON-RPC adapter must be able to serve every subsystem.
//!
//! Each capability alias states what one subsystem asks of the validator. If a
//! port trait is added to an alias but never implemented, or the adapter loses
...
/// The JSON-RPC adapter is the source every subsystem is wired to, so it has to satisfy all of them.
#[test]
