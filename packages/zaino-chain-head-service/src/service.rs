...
pub(crate) enum TipChange {
    /// New tip descends from the old one; the chain only grew
    Advance,
    /// Old tip is no longer canonical. Carries the blocks rewritten, or `None` when
    /// the fork point sits below the retained window
    Reorg(Option<u32>),
}
