...
fn header_from_zebra(zb: &zebra_chain::block::Block) -> Result<BlockHeader, ParseError> {
    let h = &zb.header;
    let height = zb
        .coinbase_height()
        .ok_or_else(|| ParseError::Deserialize("no coinbase height".into()))?;

    Ok(BlockHeader {
        hash: BlockHash::from(zb.hash().0),
        prev_hash: BlockHash::from(h.previous_block_hash.0),
        height: Height::try_from(height.0).map_err(|e| ParseError::Height(e.to_string()))?,
...
fn transparent_from_zebra(
    tx: &zebra_chain::transaction::Transaction,
) -> Result<TransparentData, ParseError> {
    let inputs = tx
        .inputs()
        .iter()
