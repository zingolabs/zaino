//! Resilience wrapper: the retry-aware decorator that turns a single-attempt
//! adapter into a source whose ports carry the retry-aware error.
//!
//! ```ignore
...
//!
//! `Resilient<V>` implements the **canonical** query ports ([`GetBlock`](crate::GetBlock),
...
//! live only here, in [`with_retry`](Resilient::with_retry).
//!
//! # The resilient ports are sealed
//!
//! Every canonical port has [`sealed::Sealed`] as a supertrait, and the only
//! implementor of `Sealed` is `Resilient<V>`. A value satisfying a canonical
...
/// Seals the canonical (resilient) ports so only [`Resilient`] can implement
/// them.
///
/// Each canonical port names `Sealed` as a supertrait; `Sealed` is implemented
/// only for [`Resilient<V>`], so "holds a resilient port" is a proof the value
/// went through [`Resilient::with_retry`], not a claim a consumer can fake.
pub(crate) mod sealed {
    /// Implemented only for [`Resilient`](super::Resilient).
    pub trait Sealed {}
    impl<V> Sealed for super::Resilient<V> {}
}
...
// The canonical (resilient) port impls for `Resilient<V>` are generated by
...
impl<V: crate::SubscribeBlocks> crate::SubscribeBlocks for Resilient<V> {
    fn subscribe_to_blocks_received(&self) -> Option<tokio::sync::watch::Receiver<()>> {
        self.inner.subscribe_to_blocks_received()
    }
}

impl<V: crate::SubscribeChainTip> crate::SubscribeChainTip for Resilient<V> {
...
mod tests {
    ...
    fn resilient_provides_the_query_ports() {
        ...
        assert_bound::<Resilient<crate::mock::MockChain>>();
    }
