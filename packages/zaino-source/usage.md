...
The resilient ports are **sealed**: the only implementor is `Resilient<V>`, so a
...
retry ladder lives in exactly one place (`Resilient::with_retry`).

```rust
use zaino_source::{GetBlock, Resilient, RetryPolicy}; // GetBlock == the resilient port
