...
[`zaino-mempool`](../../zaino-mempool/docs/usage.md) — read that first for the
...
Derive wire/compact forms from `entry.serialized_bytes()` / `entry.transaction()`
...
```rust
use zaino_mempool_rpc::CoherenceService;
