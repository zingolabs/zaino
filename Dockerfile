...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    if [ "${NO_TLS}" = "true" ]; then \
      cargo install --locked --path crates/zainod --bin zainod --root /out --features no_tls_use_unencrypted_traffic; \
    else \
      cargo install --locked --path crates/zainod --bin zainod --root /out; \
    fi
