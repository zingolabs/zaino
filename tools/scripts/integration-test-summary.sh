...
#
# Used as the script of the `live` task. Runs the `integration` then `e2e`
# partition (each in its own CI container via its own makers task), tees each
...
# partitions even when the first fails, so the summary reflects the whole suite.
# It then exits non-zero if either partition failed, so CI still catches it.
...
echo ">>> live: running integration partition"
makers live-integration 2>&1 | tee "$WL_LOG"
wl_rc=${PIPESTATUS[0]}

echo ">>> live: running e2e partition"
makers live-e2e 2>&1 | tee "$WT_LOG"
wt_rc=${PIPESTATUS[0]}
