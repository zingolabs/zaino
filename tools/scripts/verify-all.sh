...
# run on the ztest k8s harness (`cd live-tests && ztest run`), not here — they
# need a cluster, which this idempotent local check does not assume.
run_task "cargo nextest run" cargo nextest run
