use zaino_state::{
    ...
    remote_state::{RemoteStateService, RemoteStateServiceSubscriber},
};
...
async fn create_test_manager_and_services(
    ...
    let fetch_service = FetchService::spawn(config.clone()).await.unwrap();

    let fetch_subscriber = fetch_service.get_subscriber().inner();

    let remote_state_service = RemoteStateService::spawn(config).await.unwrap();

    let remote_state_subscriber = remote_state_service.get_subscriber().inner();
...
#[tokio::test]
async fn remote_state_service_check_info_regtest_no_cache_zebrad() {
    remote_state_service_check_info("zebrad", None, services::network::Network::Regtest).await;
}

#[tokio::test]
async fn remote_state_service_check_info_regtest_with_cache_zebrad() {
    remote_state_service_check_info(
        "zebrad",
...
async fn remote_state_service_check_info(
    validator: &str,
    chain_cache: Option<std::path::PathBuf>,
    ...
    let state_service_info = dbg!(remote_state_subscriber.get_info().await.unwrap());
    let state_service_blockchain_info =
        dbg!(remote_state_subscriber.get_blockchain_info().await.unwrap());
...
#[tokio::test]
async fn remote_state_service_get_address_balance_regtest_zebrad() {
...
async fn remote_state_service_get_address_balance(validator: &str) {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
    ...
    let state_service_balance = remote_state_subscriber
...
async fn remote_state_service_get_address_balance_testnet() {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
...
#[tokio::test]
async fn remote_state_service_get_block_raw_regtest_zebrad() {
...
async fn remote_state_service_get_block_raw(
    validator: &str,
    chain_cache: Option<std::path::PathBuf>,
    network: services::network::Network,
) {
    ...
    let state_service_block = dbg!(remote_state_subscriber
...
#[tokio::test]
async fn remote_state_service_get_block_object_regtest_zebrad() {
    remote_state_service_get_block_object("zebrad", None, services::network::Network::Regtest)
        .await;
}
...
async fn remote_state_service_get_block_object(
    validator: &str,
    chain_cache: Option<std::path::PathBuf>,
    network: services::network::Network,
) {
...
// #[ignore = "currently fails due to error in TrustedChainSync [https://github.com/zingolabs/zaino/issues/231]."]
#[tokio::test]
async fn remote_state_service_get_raw_mempool_regtest_zebrad() {
...
async fn remote_state_service_get_raw_mempool(validator: &str) {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
    ...
    let mut state_service_mempool = remote_state_subscriber.get_raw_mempool().await.unwrap();
...
async fn remote_state_service_get_raw_mempool_testnet() {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
...
// #[ignore = "currently fails due to error in TrustedChainSync [https://github.com/zingolabs/zaino/issues/231]."]
#[tokio::test]
async fn remote_state_service_z_get_treestate_regtest_zebrad() {
...
async fn remote_state_service_z_get_treestate(validator: &str) {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
    ...
    let state_service_treestate = dbg!(remote_state_subscriber
        .z_get_treestate("2".to_string())
        .await
        .unwrap());
...
async fn remote_state_service_z_get_treestate_testnet() {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
...
// #[ignore = "currently fails due to error in TrustedChainSync [https://github.com/zingolabs/zaino/issues/231]."]
#[tokio::test]
async fn remote_state_service_z_get_subtrees_by_index_regtest_zebrad() {
...
async fn remote_state_service_z_get_subtrees_by_index(validator: &str) {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
    ...
    let state_service_subtrees = dbg!(remote_state_subscriber
        .z_get_subtrees_by_index("orchard".to_string(), NoteCommitmentSubtreeIndex(0), None)
        .await
        .unwrap());
...
async fn remote_state_service_z_get_subtrees_by_index_testnet() {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
...
// #[ignore = "currently fails due to error in TrustedChainSync [https://github.com/zingolabs/zaino/issues/231]."]
#[tokio::test]
async fn remote_state_service_get_raw_transaction_regtest_zebrad() {
...
async fn remote_state_service_get_raw_transaction(validator: &str) {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
    ...
    let state_service_transaction = dbg!(remote_state_subscriber
        .get_raw_transaction(tx.first().to_string(), Some(1))
        .await
        .unwrap());
...
async fn remote_state_service_get_raw_transaction_testnet() {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
...
// #[ignore = "currently fails due to error in TrustedChainSync [https://github.com/zingolabs/zaino/issues/231]."]
#[tokio::test]
async fn remote_state_service_get_address_tx_ids_regtest_zebrad() {
...
async fn remote_state_service_get_address_tx_ids(validator: &str) {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
...
async fn remote_state_service_get_address_tx_ids_testnet() {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
...
async fn remote_state_service_get_address_utxos(validator: &str) {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
...
async fn remote_state_service_get_address_utxos_testnet() {
    let (
        mut test_manager,
        _fetch_service,
        fetch_service_subscriber,
