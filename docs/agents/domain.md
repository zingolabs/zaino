...
zaino is a Zcash indexer split across three Cargo workspaces — the root
`Cargo.toml` (`packages/*` production) plus the two integration-test workspaces
under `integration-tests/` — but it serves a single Zcash-indexer domain, so one
root `CONTEXT.md` + `docs/adr/` covers it. If the project later splits into
genuinely separate domains, switch to a multi-context layout (a root
`CONTEXT-MAP.md` pointing at per-area `CONTEXT.md` files) and update this file.
